...
OLDER_FIXTURE = ROOT / "tests/fixtures/update-manifest-v1.3.20.json"
...
CURRENT_PROMOTED_FIXTURE = (
    ROOT / "tests/fixtures/update-manifest-v1.3.23.json"
)
...
class UpdateManifestVerifierTests(unittest.TestCase):
    ...
    def test_current_promoted_production_root_is_valid(self) -> None:
        ...
        manifest = json.loads(raw)
        self.assertEqual(manifest["version"], "1.3.23")
        self.assertEqual(
            {
                "sourceCommit",
                "sha256SumsSha256",
