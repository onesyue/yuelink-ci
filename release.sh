...
#
# One explicitly approved official-download exception exists for v1.2.94:
#   YUELINK_OFFICIAL_UNSIGNED_CONFIRM=YUELINK_OFFICIAL_UNSIGNED_v1.2.94_2026-07-24 \
#     ./release.sh v1.2.94
...
readonly OFFICIAL_UNSIGNED_TAG="v1.2.94"
readonly OFFICIAL_UNSIGNED_UTC_DATE="2026-07-24"
readonly OFFICIAL_UNSIGNED_TOKEN="YUELINK_OFFICIAL_UNSIGNED_v1.2.94_2026-07-24"
...
if [[ "$TAG" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]]; then
  ...
  if [ "${#missing_system_signing[@]}" -gt 0 ]; then
    ...
    printf '::warning::v1.2.94 官网直发例外允许缺少 system-signing secret %s\n' \
      "${missing_system_signing[@]}"
  fi
fi
